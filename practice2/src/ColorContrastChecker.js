...
const ColorContrastChecker = () => {
  ...
  const handleCheckContrast = () => {
    ...
    if (!validColor1 || !validColor2) {
      ...
      setShowResult(false); // Hide result if any color is invalid
      return;
    }
    ...
    setShowResult(true); // Show result only if both colors are valid
  };
  ...
  return (
    <div>
      ...
      {/* Only show ratio and compliance message if both colors are valid */}
      {showResult && (
        <>
